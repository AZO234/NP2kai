...
/* 101 Mini keyboard key table */
static const SDLKCNV sdlcnvMini101[] = {
#if SDL_MAJOR_VERSION == 1
  // test result on Linux Twocode/caNnotpush/Miss/Different
  {SDLK_PAUSE,        0x60},  // STOP
  {SDLK_PRINT,        0x61},  // COPY (M)
...
/* 106 Mni keyboard key table */
static const SDLKCNV sdlcnvMini106[] = {
#if SDL_MAJOR_VERSION == 1
  // test result on Linux Twocode/caNnotpush/Miss/Different
  {SDLK_PAUSE,        0x60},  // STOP
  {SDLK_PRINT,        0x61},  // COPY (M)
...
/* Tenkey mode table */
static const SDLKCNV sdlcnvTenkey[] = {
#if SDL_MAJOR_VERSION == 1
  // test result on Linux Twocode/caNnotpush/Miss/Different
  {SDLK_7,            0x42},  // 7 ' => 7
  {SDLK_8,            0x43},  // 8 ( => 8
  ...
  {SDLK_SLASH,        0x1c},  // / ? => ENT
#else
  {SDL_SCANCODE_7,            0x42},  // 7 ' => 7
  {SDL_SCANCODE_8,            0x43},  // 8 ( => 8
  {SDL_SCANCODE_9,            0x44},  // 9 ) => 9
  {SDL_SCANCODE_0,            0x45},  // 0 0 => *
  {SDL_SCANCODE_MINUS,        0x40},  // - = => -

  {SDL_SCANCODE_U,            0x46},  // u U => 4
  {SDL_SCANCODE_I,            0x47},  // i I => 5
  {SDL_SCANCODE_O,            0x48},  // o O => 6
  {SDL_SCANCODE_P,            0x49},  // p P => +
  {SDL_SCANCODE_LEFTBRACKET,  0x41},  // [   => /

  {SDL_SCANCODE_J,            0x4a},  // j J => 1
  {SDL_SCANCODE_K,            0x4b},  // k K => 2
  {SDL_SCANCODE_L,            0x4c},  // l L => 3
  {SDL_SCANCODE_SEMICOLON,    0x4d},  // ; + => =

  {SDL_SCANCODE_M,            0x4e},  // m M => 0
  {SDL_SCANCODE_COMMA,        0x4f},  // , < => ,
  {SDL_SCANCODE_PERIOD,       0x50},  // . > => .
  {SDL_SCANCODE_SLASH,        0x1c},  // / ? => ENT
#endif
};
...
static UINT8 getKey(SDL_Scancode key) {
  ...
  if(g_tenkey) {
    imax = SDL_arraysize(sdlcnvTenkey);
    for(i = 0; i < imax; i++) {
      if(sdlcnvTenkey[i].sdlkey == key) {
        return sdlcnvTenkey[i].keycode;
      }
    }
  }
  ...
  } else if(np2oscfg.KEYBOARD == KEY_KEYMINI101) {
    sdlcnv = (SDLKCNV*)sdlcnvMini101;
    imax = SDL_arraysize(sdlcnvMini101);
  } else if(np2oscfg.KEYBOARD == KEY_KEYMINI106) {
    sdlcnv = (SDLKCNV*)sdlcnvMini106;
    imax = SDL_arraysize(sdlcnvMini106);
  } else {
